...
tar.exe -czf project.tar.gz --exclude=.git --exclude=contest-app/node_modules --exclude=contest-app/.angular --exclude=venv --exclude=__pycache__ .
...
ssh -i $SSH_KEY -o StrictHostKeyChecking=no ${USER}@${SERVER_IP} "cd $REMOTE_DIR && tar -xzf project.tar.gz && sudo docker compose --env-file .env.prod -f docker-compose.prod.yml up -d --build"
